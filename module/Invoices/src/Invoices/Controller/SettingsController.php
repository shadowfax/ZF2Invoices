...
class SettingsController extends AbstractActionController
{
    ...
    {
    	if (null === $this->taxService) {
            $this->taxService = $this->getServiceLocator()->get('invoice_service.taxes');
        }
    ...
    public function companyAction()
    {
		...
		$company = $auth->getIdentity()->getCompany();
    	$form->bind($company);
    ...
    {
    	$form = new TaxSettingsForm($this->getTaxService());
    	...
    	if ($this->request->isPost()) {
			...
			if ($form->isValid()) {
				foreach ($this->request->getPost('taxes') as $tax) {
					$taxEntity = $this->getTaxService()->find($tax['id']);
					if (empty($taxEntity)) {
						$taxEntity = new TaxEntity();	
					}
					$taxEntity->populate($tax);
					$this->getTaxService()->persist($taxEntity);
				}
